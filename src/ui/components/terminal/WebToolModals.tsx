        ...
        <div className="p-4">
          ...
          <div className="space-y-1.5">
            {containerIds.map((cid) => {
              const key = toolKey(toolId, cid)
              const isRunning = runningTools[key]?.status === "ready"
              return (
                <button
                  key={cid}
                ...
                >
                  ...
                  <span className="text-xs font-mono text-text-base flex-1">{cid}</span>
                  {isRunning && (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      ...
      >
        ...
        <div className="p-5 space-y-4">
          {WEB_TOOLS.map((tool) => {
            // Find all running instances of this tool across containers
            const instances = Object.values(runningTools).filter((r) => r.toolId === tool.id)
            return (
              <div key={tool.id} className="flex flex-col gap-2 p-3 rounded-lg bg-surface-0 border border-border-weak">
                ...
                {instances.map((inst) => (
                  <div key={`${inst.toolId}:${inst.container}`} className="flex items-center gap-2 ml-7">
                    <div className="flex-1 min-w-0">
                      {inst.status === "ready" && (
                        <span className="flex items-center gap-1 text-[10px] text-emerald-400 font-sans">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                          Running in {inst.container}
                        </span>
                      )}
                      {inst.status === "starting" && (
                        <span className="flex items-center gap-1 text-[10px] text-amber-400 font-sans">
                          <Loader2 className="w-2.5 h-2.5 animate-spin" />
                          Starting in {inst.container}...
                        </span>
                      )}
                      {inst.status === "error" && (
                        <span className="text-[10px] text-red-400 font-sans">
                          Error in {inst.container}: {inst.error?.slice(0, 60)}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      {inst.status === "ready" && (
                        <>
                          <a
                            href={getProxyUrl(inst.toolId, inst.container)}
                            target="_blank"
                          ...
                          <button
                            onClick={() => stopTool(inst.toolId, inst.container)}
