      ...
      <div className="relative w-full max-w-5xl mx-4 bg-surface-1 border border-border-base rounded-xl shadow-2xl max-h-[85vh] flex flex-col">
        ...
        <div className="flex flex-1 overflow-hidden">
          <div className="w-44 border-r border-border-weak p-2 shrink-0">
            <TabBtn active={tab === "general"} onClick={() => setTab("general")} icon={<Palette className="w-3.5 h-3.5" />} label="General" />
            <TabBtn active={tab === "providers"} onClick={() => setTab("providers")} icon={<Plug className="w-3.5 h-3.5" />} label="Providers" />
            <TabBtn active={tab === "models"} onClick={() => setTab("models")} icon={<Bot className="w-3.5 h-3.5" />} label="Models" />
            <TabBtn active={tab === "local"} onClick={() => setTab("local")} icon={<Cpu className="w-3.5 h-3.5" />} label="Local AI" />
            <TabBtn active={tab === "keybinds"} onClick={() => setTab("keybinds")} icon={<Keyboard className="w-3.5 h-3.5" />} label="Keyboard Shortcuts" />
